        ...
        public ActionResult Index(User user)
        {
            if (ModelState.IsValid)
            {
                ...
                using (CommonWealEntities context = new CommonWealEntities())
                {
                    ...
                    //var result = context.Users.Where(usr =>usr.LoginEmailID.Equals(user.LoginEmailID)&& usr.LoginPassword==user.LoginPassword).FirstOrDefault();
                    var result = context.Users.Where(usr => usr.LoginEmailID == user.LoginEmailID).FirstOrDefault();
                    if (result == null)
                    {
                        ModelState.AddModelError("", "Sorry, Commonweal doesn't recognize that email.");
                    }
                    if (result != null)
                    {
                        if (result.LoginPassword != user.LoginPassword)
                        {
